...
const MessagesList = ({ messages, username, loading, showForumName, fetchMessages }) => {
  if (username) {
  ...
  return (
    <section className={styles.messagesList}>
      {messages.map((message) => (
        <Message key={message._id} message={message} showForumName={showForumName} fetchMessages={fetchMessages} />
      ))}
    </section>
  );
};
